...
static void	ps_count(t_ps *ps)
{
	...
	ft_dprintf(2, "total : %lu instructions\n", ps->op->size);
}
...
{
	ft_dprintf(2, "usage:\t./checker [-d/-v] [values]\n");
	ft_dprintf(2, "\t-d : details, prints extra info at the end\n");
	ft_dprintf(2, "\t-v : enables viewer\n");
}
...
int			main(int argc, char **argv)
{
	...
	if (argc <= 1 || ft_strequ(argv[1], "-usage"))
		ps_usage();
	else
	{
		ps_init(&ps, &a, &b, &op);
